...
$con=mysqli_connect($host,$name,$pwd , $db) or die("connection failed");
	...
	$result=mysqli_query($con,"SELECT number_1, number_2,number_3,number_4,number_5,number_6 FROM four_nine_lottle where date between  '".$date2."' and '".$date1."' ");//user=資料表
  if (!$result) {
	 printf("Error: %s\n", mysqli_error($con));
	 exit();
	}
