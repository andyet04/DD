...
$con=mysqli_connect($host,$name,$pwd , $db) or die("connection failed");
	...
	$result=mysqli_query($con,"SELECT number_1, number_2,number_3 FROM three_star_lottle where date between  '".$date2."' and '".$date1."' ");//user=資料表
  if (!$result) {
	 printf("Error: %s\n", mysqli_error($con));
	 exit();
	}
  while($row=mysqli_fetch_array($result, MYSQLI_ASSOC)){
	...
	echo "WW";echo $v['number_1'];echo "WW";echo $v['number_2'];echo "WW";echo $v['number_3'];
	}
  mysqli_close($con);
?>
