...
$con=mysqli_connect($host,$name,$pwd , $db) or die("connection failed");
  ...
  foreach(array($tmp)[0] as $key => $v)
  {
	...
	$result=mysqli_query($con, "SELECT number_1,number_2,number_3,number_4,number_5,number_6,surprise_number FROM big_lottle where Date = '$Date[1]' ");
	$tmp = null;
  while($row=mysqli_fetch_array($result, MYSQLI_ASSOC)){
	$tmp[]=$row;
  }
  ...
  for($j=0;$j<7;$j++)
  {
	  ...
	  (
		  ...
		  or number_3 = $first[$j] or number_4 = $first[$j] or number_5 = $first[$j]
		   or number_6 = $first[$j] or surprise_number = $first[$j]) and Date between '$Date[$count]' and '$Date[2]'
	   )");
	  if (!$result) {
		 printf("Error: %s\n", mysqli_error($con));
		 exit();
